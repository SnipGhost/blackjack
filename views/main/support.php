<div class="content-box-opi">
	<div class="content-box-title">
		Поддержка
	</div>
<!--
	<div class="content-box-semititle">
			Репутационный менеджмент
	</div>	
-->
    <div class="small-description">
        <br>
        Мы очень благодарны за помощь в улучшении сайта! Если Вы заметили ошибку, то просим сообщить о ней.
    </div>
</div>
<!--<div class="decorative-strip"></div>-->
<div class="content-box">
    <div class="contacts-support-block">
        <li>Отправить письмо на электронную почту: support_sturtup@gmail.com</li>
        <li>Связаться во ВКонтакте: <a href=">https://vk.com/support">https://vk.com/support</a></li>
        <li>Другие ресурсы</li>
    </div>
</div>